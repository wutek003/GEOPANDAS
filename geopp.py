...
gdw=gdw.to_crs("EPSG:4326")
cell = gpd.GeoDataFrame(gdw, columns=['geometry'])
ax=gdf.plot(markersize=.1, figsize=(12, 8), column='TOT_MALE_65', cmap='jet')
plt.autoscale(False)
cell.plot(ax=ax, facecolor="none", edgecolor='grey')
ax.axis("off")
merged = gpd.sjoin(gdf, cell, how='left', op='within')
dissolve = merged.dissolve(by="index_right", aggfunc="sum")
cell.loc[dissolve.index, 'TOT_MALE_65'] = dissolve.TOT_MALE_65.values
ax = cell.plot(column='TOT_MALE_65', figsize=(12, 8), cmap='viridis', edgecolor="grey", legend = True)
plt.autoscale(True)
ax.set_axis_on()
plt.axis('equal');
...
ax.set_axis_on()
plt.axis('equal');
plt.title('Liczba kobiet w wieku większym niż 65 lat w poszczególnych województwach')
